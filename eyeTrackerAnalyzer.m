...
% parameters are saved in an xml format
% the following constants store the xml nodes names
...
MICROSACCADES_ANALYSIS_DETECTION_XML_NODE_NAME = 'DETECTION';
...
% maximum number of panels openned at one time in the 
% 'create .ETAs - multi sessioned' screen
MAX_SUBJECTS_NR_FOR_ETAS_CREATION= 10;

% default degrees per pixel
DPP= 1/60;

% global variables to store the parameters taken by the GUI
...
ENGBERT_ALGORITHM_DEFAULTS.detection = EyeTrackerAnalysisRecord.ENUM_DETECTION_BINOCULAR;
...
% ------------------------------------------------- %
% --- DROPDOWN MENU FOR SELECTION OF A GUI PANE --- %
% ------------------------------------------------- %
% the 'Action' button in the window's menu-bar to open the dropdown menu
menu_bar= uimenu(gui,'Label','Action');
% Action dropdown option: Eye Data Analysis pane
...
% Action dropdown option: Multi-Sessioned .ETAs creation pane
...
% Action dropdown option: Single-Sessioned .ETAs creation pane
...
% Action dropdown option: Extraction of the eye data as a Matlab struct pane
...
% ------------------------------ %
% --- EYE DATA ANALYSIS PANE --- %
% ------------------------------ %
% A matlab's decorative gui panel
...
% ----------------------------------------------- %
% --- LOAD SUBJECTS' .ETAs FOR ANALYSIS PANEL --- %
% ----------------------------------------------- %
% loading images for the add and remove file buttons
...
% a global to keep track of the number of .ETAs files added to the panel
% (and thus the number of subjects requested to be analyzed)
subjects_nr= 0;
...
% the pane to show and select .ETAs
...
% ONSET TRIGGERS GUI CONTROLS
% grouping the onset triggers controls within a borderless invisible panel
...
% the panel to display and select triggers
...
% button for deletion of triggers that are selected in the display panel
...
% text box for typing in an onset trigger (or an onset triggers pattern regex)
...
% OFFSET TRIGGERS GUI CONTROLS
% grouping the offset triggers controls within a borderless invisible panel
trials_offset_triggers_button_group= uipanel(segmentation_panel, 'tag', 'c72', ...
    'Position', [0.22635     0.0360     0.2206     0.9063], ...
    'Background', GUI_BACKGROUND_COLOR, ...
    'BorderWidth', 0);

% offset triggers controls title
...
% text box for typing in an offset trigger (or an offset triggers pattern regex)
...
% REJECTION TRIGGERS GUI CONTROLS
% grouping the rejection triggers controls within a borderless invisible panel
trials_rejection_triggers_button_group= uipanel(segmentation_panel, 'tag', 'c75', ...
    'Position', [0.4494    0.0360    0.2206    0.9063], ...
    'Background', GUI_BACKGROUND_COLOR, ...
    'BorderWidth', 0);

% rejection triggers controls title
...
% text box for typing in a rejection trigger (or an offset triggers pattern regex)
...
% SEGMENTATION EXTRA TIME-FRAMES PARAMETERS
% pre-onset-trigger control title
uicontrol(segmentation_panel, 'Style', 'text', 'tag', 'c12', 'units', 'normalized', ...
    'String', 'Pre-onset-trigger segment duration (ms)', ...
    'Position', [0.6798, 0.5228, 0.1198, 0.3254], ...
    'FontSize', 10.0, ...
    'BackgroundColor', GUI_BACKGROUND_COLOR);

% pre-onset-trigger text box
...
% post-offset-trigger control title
post_offset_trigger_segment_txt_uicontrol = uicontrol(segmentation_panel, 'Style', 'text', 'tag', 'c73', 'units', 'normalized', ...
    ...
    'Position', [0.6799, 0.1479, 0.1198, 0.3114], ...
    'FontSize', 10.0, ...
    'Enable', 'off', ...
    'BackgroundColor', GUI_BACKGROUND_COLOR);

% post-offset-trigger text box
...
% analysis segment duration text box
...
% -------------------------------------- %
% save parameters values to file button
...
% load parameters values from file button
...
uicontrol(analyze_microsaccades_panel, 'Style', 'text', 'tag', 'c95', 'units', 'normalized', ...
    'String', 'Analysis Output Folder', ...
    'Position', [0.075    0.5146    0.18    0.0300], ...
    'FontSize', 12.0, ...
    'BackgroundColor', GUI_BACKGROUND_COLOR);

% button to fire up the folder selection dialogue
...
% text box to display the selected folder 
...
experiment_screen_ppd_editbox_label = uicontrol(etas_creation_multi_sessioned_panel, 'Style','text', 'tag', 'c4.0', 'units', 'normalized', ...
    'String', 'Experiment Screen''s Pixels Per Visual Degree', ...
    'Position', [0.0541    0.0615    0.2192    0.0545], ...
    'FontSize', 14.0, ...
    'BackgroundColor', GUI_BACKGROUND_COLOR);

experiment_screen_ppd_editbox = uicontrol(etas_creation_multi_sessioned_panel, 'Style', 'edit', 'tag', 'c4.1', 'units', 'normalized', ...
    'string', 1/DPP, ...
...
% ------------------------------------------------------ %
% --- .ETAs CREATION FROM INDIVIDUAL DATA FILES PANE --- %
% ------------------------------------------------------ %
% pane's title
...
uicontrol(etas_creation_single_sessioned_panel, 'Style', 'text', 'tag', 'c100', 'units', 'normalized', ...
    'String', 'Create .ETAs From Eye tracking Files', ...
    'Position', [0.215574385964911, 0.899309090909091, 0.628076022933653, 0.071823206086844], ...
    'FontSize', 24.0, ...    
    'BackgroundColor', GUI_BACKGROUND_COLOR);
...
%=================%
    ...
    function guiActionSelectedCreateEtasMultiSessioned(~,~)
        ...
        set(experiment_screen_ppd_editbox_label, 'Parent', etas_creation_multi_sessioned_panel);
        set(experiment_screen_ppd_editbox, 'Parent', etas_creation_multi_sessioned_panel);
    end        
    ...
    function guiActionSelectedCreateEtasSingleSessioned(~,~)
        ...
        set(experiment_screen_ppd_editbox_label, 'Parent', etas_creation_single_sessioned_panel);
        set(experiment_screen_ppd_editbox, 'Parent', etas_creation_single_sessioned_panel);
    end        
    ...
    function saveParamsBtnCallback(~, ~)
        ...
        %if isempty(xml_dom.getElementsByTagName(MICROSACCADES_ANALYSIS_PARAMS_NODE_NAME).item(0))
            ...
            createXmlNode(xml_dom, saccades_analysis_params_xml_node, MICROSACCADES_ANALYSIS_DETECTION_XML_NODE_NAME);
        ...
        setXmlNodeValue(xml_dom, MICROSACCADES_ANALYSIS_DETECTION_XML_NODE_NAME, num2str(ENGBERT_ALGORITHM_DEFAULTS.detection));
    ...
    function loadParamsBtnCallback(~, ~)
        ...
        ENGBERT_ALGORITHM_DEFAULTS.detection = valIfValidDefaultIfNaN(str2double(getXmlNodeValue(xml_dom, MICROSACCADES_ANALYSIS_DETECTION_XML_NODE_NAME)), ENGBERT_ALGORITHM_DEFAULTS.detection);
        ...
        if ~isempty(MICROSACCADES_PARAMETERS_FIG) && isobject(MICROSACCADES_PARAMETERS_FIG)            
            ...
            set(findobj('tag', 'c900'), 'UserData', ENGBERT_ALGORITHM_DEFAULTS.detection);
            set(findobj('tag', 'c9001'), 'Value', ENGBERT_ALGORITHM_DEFAULTS.detection == EyeTrackerAnalysisRecord.ENUM_DETECTION_BINOCULAR);
            set(findobj('tag', 'c9002'), 'Value', ENGBERT_ALGORITHM_DEFAULTS.detection == EyeTrackerAnalysisRecord.ENUM_DETECTION_MONOCULAR_L);
            set(findobj('tag', 'c9003'), 'Value', ENGBERT_ALGORITHM_DEFAULTS.detection == EyeTrackerAnalysisRecord.ENUM_DETECTION_MONOCULAR_R);                        
            set(findobj('tag', 'c205'), 'string', num2str(ENGBERT_ALGORITHM_DEFAULTS.vel_threshold));
            set(findobj('tag', 'c206'), 'Value', MICROSACCADES_ANALYSIS_PARAMETERS.rate);
...
%         end           
    ...
    function analysis_go= microsaccadesParametersFigCreator()
        ...
        MICROSACCADES_PARAMETERS_FIG= figure('Visible', 'on', 'name', 'Eye Movements Analysis Parameters', 'NumberTitle', 'off', 'units', 'pixels', ...
            'Position', [main_figure_positions(1), main_figure_positions(2) + 0.1*main_figure_positions(4), main_figure_positions(3), 0.8*main_figure_positions(4)], ...
            'MenuBar', 'none', ... 
        ...
        uicontrol(microsaccades_analysis_params_panel, 'Style', 'text', 'tag', 'c500', 'units', 'normalized', ...
            'String', 'Min saccade amplitude [deg]', ...
            'Position', [0.422,0.5568,0.1688,0.1307], ...
            'FontSize', 10.0, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);
        
        uicontrol(microsaccades_analysis_params_panel, 'Style', 'edit', 'tag', 'c501', 'units', 'normalized', ...
            'Position', [0.6246,0.5794,0.0671,0.1008], ...
        ...
        gen_single_graphs_checkbox = uicontrol(microsaccades_analysis_params_panel, 'Style', 'checkbox', 'tag', 'c230', 'units', 'normalized', ...
            'FontSize', 10.0, 'String', 'Plot individual figures', 'Position', [0.0322,0.1476,0.2477,0.0923], ...
            'BackgroundColor', GUI_BACKGROUND_COLOR, ...
        ...
        gen_group_graphs_checkbox = uicontrol(microsaccades_analysis_params_panel, 'Style', 'checkbox', 'tag', 'c231', 'units', 'normalized', ...
            'FontSize', 10.0, 'String', 'Plot group figures', 'Position', [0.0322,0.0496,0.2477,0.0923], ...
            'BackgroundColor', GUI_BACKGROUND_COLOR, ...
        ...
        uicontrol(microsaccades_analysis_params_panel, 'Style', 'checkbox', 'tag', 'c311', 'units', 'normalized', ...
            'FontSize', 10.0, 'String', 'Extract pupil size', 'Position', [0.0322,0.2756,0.2466,0.0923], ...
            'BackgroundColor', GUI_BACKGROUND_COLOR, ...
        ...
        save_rate_graphs_checkbox = uicontrol(microsaccades_analysis_params_panel, 'Style', 'checkbox', 'tag', 'c206', 'units', 'normalized', ...
            'FontSize', 10.0, 'String', 'Extract saccade rate', 'Position', [0.0322,0.8659,0.2353,0.0925], ...
            'BackgroundColor', GUI_BACKGROUND_COLOR, ...
        ...
        smoothing_window_len_edit_text= uicontrol(microsaccades_analysis_params_panel, 'Style', 'text', 'tag', 'c2061', 'units', 'normalized', ...
            'String', 'Smoothing window length [ms]', ...
            'Position', [0.019632142857142,0.734304750000004,0.210034374999997,0.111995250000003], ...
            'FontSize', 10.0, ...
            'Enable', are_save_graphs_enabled_str, ... 
            'BackgroundColor', GUI_BACKGROUND_COLOR);
        
        smoothing_window_len_edit= uicontrol(microsaccades_analysis_params_panel, 'Style', 'edit', 'tag', 'c2062', 'units', 'normalized', ...
            'Position', [0.2421,0.7403,0.0672,0.1018], ...
        ...
        save_amplitude_graphs_checkbox = uicontrol(microsaccades_analysis_params_panel, 'Style', 'checkbox', 'tag', 'c207', 'units', 'normalized', ...
            'FontSize', 10.0, 'String', 'Extract saccade amplitudes', 'Position', [0.0322,0.619800000000001,0.2811,0.0859], ...
            'BackgroundColor', GUI_BACKGROUND_COLOR, ...
        ...
        save_direction_graphs_checkbox = uicontrol(microsaccades_analysis_params_panel, 'Style', 'checkbox', 'tag', 'c208', 'units', 'normalized', ...
            'FontSize', 10.0, 'String', 'Extract saccade directions', 'Position', [0.0322,0.5036,0.2699,0.0924], ...
            'BackgroundColor', GUI_BACKGROUND_COLOR, ...
        ...
        save_main_seq_graphs_checkbox = uicontrol(microsaccades_analysis_params_panel, 'Style', 'checkbox', 'tag', 'c400', 'units', 'normalized', ...
            'FontSize', 10.0, 'String', 'Extract main sequence', 'Position', [0.0322,0.4104,0.3123,0.0562], ...
            'BackgroundColor', GUI_BACKGROUND_COLOR, ...
        ...
        uicontrol(microsaccades_analysis_params_panel, 'Style', 'text', 'tag', 'c210', 'units', 'normalized', ...
            'String', 'Min saccade duration [ms]', ...
            'Position', [0.4300,0.1829,0.1548,0.0917], ...
            'FontSize', 10.0, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);

        uicontrol(microsaccades_analysis_params_panel, 'Style', 'edit', 'tag', 'c211', 'units', 'normalized', ...
            'Position', [0.6276,0.1737,0.0673,0.1009], ...
        ...
        uicontrol(microsaccades_analysis_params_panel, 'Style', 'text', 'tag', 'c212', 'units', 'normalized', ...
            'String', 'Min inter-saccade duration [ms]', ...
            'Position', [0.4133,0.3093,0.1907,0.0987], ...
            'FontSize', 10.0, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);

        uicontrol(microsaccades_analysis_params_panel, 'Style', 'edit', 'tag', 'c213', 'units', 'normalized', ...
            'Position', [0.6276,0.3072,0.0673,0.1009], ...
        ...
        uicontrol(microsaccades_analysis_params_panel, 'Style', 'text', 'tag', 'c214', 'units', 'normalized', ...
            ...
            'Position', [0.4064,0.0696,0.1926,0.0478], ...
            'FontSize', 10.0, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);

        uicontrol(microsaccades_analysis_params_panel, 'Style', 'edit', 'tag', 'c215', 'units', 'normalized', ...
            'Position', [0.6276,0.0329,0.0673,0.1009], ...
            'string', num2str(ENGBERT_ALGORITHM_DEFAULTS.filter_bandpass), ...
            'callback', {@filterBandpassEditedCallback});
        
        general_analysis_params_panel= uipanel(MICROSACCADES_PARAMETERS_FIG, 'tag', 'p12', 'units', 'normalized', ...
            'Position',[0.6948    0.3991    0.3005    0.5838], ...
            'visible', 'on', ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);
        
        uicontrol(general_analysis_params_panel, 'Style', 'checkbox', 'tag', 'c209', 'units', 'normalized', ...
            'FontSize', 10.0, 'String', 'Perform visual data inspection', 'Position', [0.0685, 0.1732, 0.8641, 0.0400], ...
            'BackgroundColor', GUI_BACKGROUND_COLOR, ...
            'value', PERFORM_EYEBALLING, ...
            'callback', {@eyeballMicrosaccadesToggledCallback});  
        
        eyeballer_display_range_text = uicontrol(general_analysis_params_panel, 'Style', 'text', 'tag', 'c216', 'units', 'normalized', ...
            'String', 'Data inspection display range [std from mean]', ...
            'Position', [0.0112, 0.0494, 0.7269, 0.0944], ...
            'FontSize', 10.0, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);
        
        eyeballer_display_range_edit = uicontrol(general_analysis_params_panel, 'Style', 'edit', 'tag', 'c217', 'units', 'normalized', ...
            'Position', [0.7780, 0.0478, 0.1772, 0.0952], ...
            'string', num2str(EYEBALLER_DISPLAY_RANGE), ...
            'callback', {@eyeballerDisplayRangeEditedCallback});
        
        detection_combo_button_group= uibuttongroup(general_analysis_params_panel, 'tag', 'c900', ...
            'Position', [0.0284, 0.7519, 0.9486, 0.2139], ...         
            'UserData', ENGBERT_ALGORITHM_DEFAULTS.detection, ...
            'SelectionChangeFcn', {@detectionComboCallback}, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);
                       
        uicontrol(detection_combo_button_group, 'Style','text', 'units', 'normalized', ... 
            'String', 'Saccade detection', ... 
            'Position', [0.0222, 0.2872, 0.3640, 0.5400], ... 
            'FontSize', 10.0, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);    
       
        uicontrol(detection_combo_button_group, 'Style','radiobutton', ...
            'tag', 'c9001', 'units', 'normalized', 'String', 'Binocular', ...
            'Value', ENGBERT_ALGORITHM_DEFAULTS.detection == EyeTrackerAnalysisRecord.ENUM_DETECTION_BINOCULAR, ...
            'Position', [0.4058 0.6681 0.5129 0.2000], ...
            'FontSize', 10.0, ... 
            'UserData', EyeTrackerAnalysisRecord.ENUM_DETECTION_BINOCULAR, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);
                      
        uicontrol(detection_combo_button_group, 'Style','radiobutton', ...
            'tag', 'c9002', 'units', 'normalized', 'String', 'Monocular (L)', ...
            'Value', ENGBERT_ALGORITHM_DEFAULTS.detection == EyeTrackerAnalysisRecord.ENUM_DETECTION_MONOCULAR_L, ...
            'Position', [0.4072 0.3885 0.4992 0.2000], ...
            'FontSize', 10.0, ... 
            'UserData', EyeTrackerAnalysisRecord.ENUM_DETECTION_MONOCULAR_L, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);
        
        uicontrol(detection_combo_button_group, 'Style','radiobutton', ...
            'tag', 'c9003', 'units', 'normalized', 'String', 'Monocular (R)', ...
            'Value', ENGBERT_ALGORITHM_DEFAULTS.detection == EyeTrackerAnalysisRecord.ENUM_DETECTION_MONOCULAR_R, ...
            'Position', [0.4072 0.0783 0.5611 0.2000], ...
            'FontSize', 10.0, ... 
            'UserData', EyeTrackerAnalysisRecord.ENUM_DETECTION_MONOCULAR_R, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);
                                
        blinks_detection_algos_flags_combo_button_group= uibuttongroup(general_analysis_params_panel, 'tag', 'c815', ...
            'Position', [0.028, 0.5079, 0.9491, 0.2139], ...         
            'UserData', MICROSACCADES_ANALYSIS_PARAMETERS.blinks_detection_algos_flags, ...                              
            'SelectionChangeFcn', {@blinksDetectionAlgosFlagsComboCallback}, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);
                       
        uicontrol(blinks_detection_algos_flags_combo_button_group, 'Style','text', 'units', 'normalized', ... 
            'String', 'Blink detection algorithm', ... 
        ...
        uicontrol(blinks_detection_algos_flags_combo_button_group, 'Style','radiobutton', ...
            'tag', 'c8151', 'units', 'normalized', 'String', 'SR Research', ...
        ...
        uicontrol(blinks_detection_algos_flags_combo_button_group, 'Style','radiobutton', ...
            'tag', 'c8153', 'units', 'normalized', 'String', 'Both', ...
        ...
        uicontrol(general_analysis_params_panel, 'Style', 'text', 'tag', 'c80', 'units', 'normalized', ...
            'String', 'Blinks delta [ms]', ...
            'Position', [0.0279, 0.4066, 0.6856, 0.0567], ...
            'FontSize', 10.0, ...
            'BackgroundColor', GUI_BACKGROUND_COLOR);

        uicontrol(general_analysis_params_panel, 'Style', 'edit', 'tag', 'c811', 'units', 'normalized', ...
            'Position', [0.7780, 0.3823, 0.1772, 0.0952], ...
        ...
        function blinksDetectionAlgosFlagsComboCallback(~, eventdata)                                                                 
            MICROSACCADES_ANALYSIS_PARAMETERS.blinks_detection_algos_flags= get(eventdata.NewValue, 'UserData');
        end
        
        function detectionComboCallback(~, eventdata)
            ENGBERT_ALGORITHM_DEFAULTS.detection= get(eventdata.NewValue, 'UserData');
        end
