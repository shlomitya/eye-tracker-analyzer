...
float getMappedPPDX (float x);
float getMappedPPDY (float y);
#else
#define getMappedX(x) (x)
#define getMappedY(y) (y)
#define getMappedPPDX(x) (x)
#define getMappedPPDY(y) (y)
#endif
...
int print_event(FEVENT *fevent, int sample_int)
{
  ...
  switch(etype)
  {
      ...
      case STARTBLINK:
		...
		if(options.use_tabs)
			print("SBLINK\t%c\t%u%s\n", fevent->eye?'R':'L', fevent->sttime,(options.out_float_time?".0":""));     //Note: %lu --> %u for unsigned int
		else
			print("SBLINK %c %u%s\n", fevent->eye?'R':'L', fevent->sttime,(options.out_float_time?".0":""));       //Note: %lu --> %u for unsigned int
		break;
	  ...
	  case ENDBLINK:
		  ...
		  {
			  printf("###ENDBLINK end time is less than start time %u%s %u%s\n",fevent->sttime,(options.out_float_time?".0":""), fevent->entime,(options.out_float_time?".0":""));        //Note: %lu --> %u for unsigned int
		  }
		  eventpairs[0][1][fevent->eye] = fevent->sttime;
		if(options.use_tabs)
			print("EBLINK\t%c\t%u%s\t%u%s\t%u%s\n", fevent->eye?'R':'L',
				fevent->sttime, (options.out_float_time?".0":""),fevent->entime,(options.out_float_time?".0":""), fevent->entime - fevent->sttime + (UINT32)sample_int,(options.out_float_time?".0":""));              //Note: %lu --> %u for unsigned int
		else
			print("EBLINK %c %u%s\t%u%s\t%u%s\n", fevent->eye?'R':'L',
				fevent->sttime, (options.out_float_time?".0":""),fevent->entime,(options.out_float_time?".0":""), fevent->entime - fevent->sttime + (UINT32)sample_int,(options.out_float_time?".0":""));              //Note: %lu --> %u for unsigned int
		break;
	  ...
	  case ENDSACC:
		  ...
		  {
			  printf("###ENDSACC end time is less than start time %u%s %u%s\n",fevent->sttime,(options.out_float_time?".0":""), fevent->entime,(options.out_float_time?".0":""));          //Note: %lu --> %u for unsigned int
		  }
		  eventpairs[1][1][fevent->eye] = fevent->sttime;
		if(options.use_tabs)
			print("ESACC\t%c\t%u%s\t%u%s\t%u%s", fevent->eye?'R':'L',
				fevent->sttime, (options.out_float_time?".0":""), fevent->entime,(options.out_float_time?".0":""), fevent->entime - fevent->sttime + (UINT32)sample_int,(options.out_float_time?".0":""));               //Note: %lu --> %u for unsigned int
		else
			print("ESACC %c  %u%s\t%u%s\t%u%s", fevent->eye?'R':'L',
				fevent->sttime,(options.out_float_time?".0":""), fevent->entime,(options.out_float_time?".0":""),
				fevent->entime - fevent->sttime + (UINT32)sample_int,(options.out_float_time?".0":""));                                               //Note: %lu --> %u for unsigned int
		switch(options.output_event_type)
		{
			...
			{
				print_value(hypot(
					(getMappedX(fevent->gstx)-getMappedX(fevent->genx))/(getMappedPPDX((fevent->supd_x+fevent->eupd_x)/2.0)),
					(getMappedY(fevent->gsty)-getMappedY(fevent->geny))/(getMappedPPDY((fevent->supd_y+fevent->eupd_y)/2.0))), 2);
	 ...
	 switch(options.output_event_type)
	   {
			...
			default:
			  if((fevent->read&READ_GRES) && !options.force_resolution)
				{
					print_value(getMappedPPDX(fevent->supd_x+fevent->eupd_x)/2.0, 2);
					print_value(getMappedPPDY(fevent->supd_y+fevent->eupd_y)/2.0, 2);
				}
			  else
				{
	...
	break;
	  ...
	  case ENDFIX:
		  if(fevent->entime < fevent->sttime)
		  {
			  printf("###ENDFIX end time is less than start time %u %u\n",fevent->sttime, fevent->entime);          //Note: %lu --> %u for unsigned int
		  }
		  eventpairs[2][1][fevent->eye] = fevent->sttime;
		if(options.use_tabs)
			print("EFIX\t%c\t%u%s\t%u%s\t%u%s", fevent->eye?'R':'L',
      ...
      case BUTTONEVENT:
			...
			for(i=1;i<9;i++)
			{
				if(b&1)
				{
					print("BUTTON\t%u%s\t%d\t%d\n", fevent->sttime,(options.out_float_time?".0":""), i, bs&1);       //Note: %lu --> %u for unsigned int
				}
				b  >>= 1;
				bs >>= 1;
			}
      ...
      case MESSAGEEVENT:
			...
			trim_lstring(fevent->message, 250);
			if(options.use_tabs)
				print("MSG\t%u%s\t%s\n", fevent->sttime,(options.out_float_time?".0":""), &(fevent->message->c));           //Note: %lu --> %u for unsigned int
			else
				print("MSG\t%u%s %s\n", fevent->sttime,(options.out_float_time?".0":""), &(fevent->message->c));            //Note: %lu --> %u for unsigned int
			break;
      case INPUTEVENT:
            print("INPUT\t%u%s\t%d\n", fevent->sttime,(options.out_float_time?".0":""), fevent->input);                     //Note: %lu --> %u for unsigned int
            break; 
      default:
			break;
  }
			...
			if(eventpairs[i][0][j] != -1 && eventpairs[i][1][j] != -1)
			{
				if(eventpairs[i][0][j] != eventpairs[i][1][j])
				{
					printf("start time miss match %s %u %u %d\n", (i==0)? "BLINK" : (i ==1)? "SAC" :"FIX",
						eventpairs[i][0][j], eventpairs[i][1][j],eventpairs[i][1][j]-eventpairs[i][0][j]);                     //Note: %lu --> %u for unsigned int
				}
