classdef EyeTrackerAnalysisRecord < handle
    ...
    methods (Access= public)
        function obj= EyeTrackerAnalysisRecord(analysis_tag, eye_tracker_files, dpp, progress_tracking_callback_func, progress_contribution)             
            [code_folder, ~, ~] = fileparts(mfilename('fullpath'));
            obj.read_edf_path = fullfile(code_folder, 'readEDF'); 
            ...
            for eye_tracker_file_i= 1:eye_tracker_files_nr                 
                ...
                if strcmp(eye_tracker_file_ext, '.edf')                    
                    copyfile(curr_eye_tracker_full_file_name, obj.read_edf_path);                    
                    addpath(obj.read_edf_path);
                    extracted_struct = readEDF(fullfile(obj.read_edf_path, [eye_tracker_file_name, '.edf']));
                    ...
                    delete(fullfile(obj.read_edf_path, [eye_tracker_file_name, '.edf']));
                    rmpath(obj.read_edf_path);                  
                ...
                if nargin > 3 && ~isempty(progress_tracking_callback_func)
                    progress_tracking_callback_func(progress_contribution/eye_tracker_files_nr);
                end
                obj.eye_tracker_data_structs= [obj.eye_tracker_data_structs, extracted_structs];                  
            end                         
        end                
                  
        function was_previous_segmentization_loaded = segmentizeData(obj, progress_tracking_callback_func, progress_contribution, trial_onset_triggers, trial_offset_triggers, trial_rejection_triggers, baseline, post_offset_triggers_segment, trial_dur, blinks_delta, blink_detection_algos_flags)
            ...
            for session_i= 1:sessions_nr
                ...
                if blink_detection_algos_flags(1)
                    eyelink_based_blinks_vec = EyeTrackerAnalysisRecord.eyelinkBased_blinkdetection(curr_session_eye_tracker_data_struct, blinks_delta, progress_tracking_callback_func, 0.4*progress_contribution/sessions_nr);
                else
                ...
                for trigger_i= 1:triggers_nr                                        
                    ...
                    if isempty(start_times)
                        ...
                        if isempty(start_times)
                            ...
                            if ~isempty(progress_tracking_callback_func)
                                progress_tracking_callback_func(0.2*progress_contribution/(sessions_nr*triggers_nr));
                            end
                            continue;
                        end                                     
                    end
        ...
        function segmentized_data= getSegmentizedData(obj, progress_tracking_callback_func, progress_contribution, filter_bandpass)
            segmentized_data = [];
            ...
            for session_i= 1:sessions_nr  
                ...
                for cond_name_i= 1:numel(conds_names)
                    ...
                    if trials_nr==0
                        ...
                        if ~isempty(progress_tracking_callback_func)
                            progress_tracking_callback_func(progress_contribution / (numel(conds_names) * sessions_nr));
                        end
                    else                        
                        for trial_i= 1:trials_nr                           
                            ...
                            if ~isempty(progress_tracking_callback_func) && mod(trial_i, 50) == 0
                                progress_tracking_callback_func(progress_contribution / (numel(conds_names) * sessions_nr * (trials_nr/50)));
                            end
                        end
                        
                        if ~isempty(progress_tracking_callback_func)
                            progress_tracking_callback_func(progress_contribution * mod(trials_nr, 50) / (numel(conds_names) * sessions_nr * trials_nr));
                        end
                    end                                        
                end
            end
    ...
    methods (Access= private, Static)                    
        ...
        function blinksbool= eyelinkBased_blinkdetection(eyelink, delta, progress_tracking_callback_func, progress_contribution)
            if nargin==1
                delta=130;
            end
            ...
            for i= 1:blinks_nr
                if ~isempty(progress_tracking_callback_func) && mod(i,interval_blinks_nr)==0
                    progress_tracking_callback_func(interval_progress_contribution);
                end
            ...
            if ~isempty(progress_tracking_callback_func) && mod(blinks_nr,interval_blinks_nr)~=0
                progress_tracking_callback_func(progress_contribution*mod(1,interval_blinks_nr/blinks_nr));
            end
        end 
        
        function new_blink_vec=pupilBased_blinkdetection_twoEyes(pupilr, pupill, Fs, std, consq_samples, tolerance, padding, maxsegtime, progress_tracking_callback_func, progress_contribution)
            %% inputs
            ...
            if (length(pupilr)/(maxsegtime*Fs))>1
                ...
                for i=1:length(starttimes)                    
                    ...
                    [blink_vecr,problemflagr]=EyeTrackerAnalysisRecord.pupilBased_blinkdetection(cur_pupilr,Fs,std,consq_samples,tolerance, progress_tracking_callback_func, 0.5*progress_contribution/length(starttimes));
                    [blink_vecl,problemflagl]=EyeTrackerAnalysisRecord.pupilBased_blinkdetection(cur_pupill,Fs,std,consq_samples,tolerance, progress_tracking_callback_func, 0.5*progress_contribution/length(starttimes));
                    ...
                    if ~isempty(progress_tracking_callback_func)
                        progress_tracking_callback_func(progress_contribution/length(starttimes));
                    end
                end                
            else
                [new_blink_vecr,problemflagr]=EyeTrackerAnalysisRecord.pupilBased_blinkdetection(pupilr,Fs,std,consq_samples,tolerance, progress_tracking_callback_func, 0.5*progress_contribution);
                [new_blink_vecl,problemflagl]=EyeTrackerAnalysisRecord.pupilBased_blinkdetection(pupill,Fs,std,consq_samples,tolerance, progress_tracking_callback_func, 0.5*progress_contribution);
        ...
        function [blink_vec,problemflag]=pupilBased_blinkdetection(pupildata,Fs,std,consq_samples,tolerance, progress_tracking_callback_func, progress_contribution)
