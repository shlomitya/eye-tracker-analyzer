...
classdef SaccadesExtractor < handle
    ...
    methods (Access= public)       
        ...
        function [eye_data_struct, saccades_struct]= extractSaccadesByEngbert(obj, vel_calc_type, vel_threshold, amp_lim, amp_low_lim, saccade_dur_min, frequency_max, filter_bandpass, etas_full_paths, progress_contribution, progress_tracking_callback_func)                                       
            ...
            for subject_i= 1:obj.subjects_nr
                subjects_data_structs{subject_i}= obj.subjects_etas{subject_i}.getSegmentizedData(progress_tracking_callback_func, 0.8*progress_contribution/obj.subjects_nr, curr_requested_low_pass_filter);
            ...
            for subject_i= 1:obj.subjects_nr
                if ~isempty(saccades_struct{subject_i})
                    if nargin == 11 && ~isempty(progress_tracking_callback_func)
                        progress_tracking_callback_func(0.2*progress_contribution/obj.subjects_nr);
                    end
                    continue;
                end
                ...
                for cond_i= 1:numel(conds_names)
                    ...
                    for trial_i= 1:numel(curr_cond_struct)
                        ...
                        saccades_struct{subject_i}.(curr_cond_name)(trial_i).directions= -curr_trial_directions / pi * 180;
